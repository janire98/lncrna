...
rutaArchivo <- "/Users/janire/Desktop/TFM/TFM_workflow/005_clusterings/Subread_100_counts_normal"
files <- list.files(rutaArchivo, pattern=".csv", recursive=TRUE, full.names=TRUE)

filtroMinimoPacientes <- as.numeric(6)

rutaGuardado <- "/Users/janire/Desktop/TFM/TFM_workflow/006_survival_analysis/Subread/Subread_100_normalized"


# Import t_rog dataset rescaled and make the hierarchical clustering
for (file in files) {
rlog_rescalado <- read_csv(file)
rlog_rescalado <- tibble::column_to_rownames(rlog_rescalado, var="X1")
